...
export default function LibrarianEditRoomPage() {
    ...
    async function load() {
      try {
        ...
        setInitialValues({
          ...
          amenityPowerOutlets: !!amenities.powerOutlets,
        });
      } catch {
        addToast("Error al cargar sala", "danger");
        router.push("/dashboard/librarian/rooms");
      } finally {
        setLoading(false);
      }
    }
  ...
  async function handleSubmit(data: RoomFormData) {
    ...
    if (!res.ok) {
      ...
      throw new Error(err.error || "Error al actualizar sala");
    }

    addToast("Sala actualizada correctamente", "success");
    router.push("/dashboard/librarian/rooms");
  }
  ...
  return (
    <div className="space-y-6">
      <BackLink href="/dashboard/librarian/rooms" label="Volver a salas" />
      <h1 className="text-2xl font-bold text-text">Editar sala</h1>
      <Card className="max-w-2xl mx-auto p-6">
        <RoomForm onSubmit={handleSubmit} initialValues={initialValues} isEdit />
      </Card>
    </div>
  );
}
