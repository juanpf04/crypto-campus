...
export default function AdminNewItemPage() {
  ...
  async function handleSubmit(data: ItemFormData) {
    ...
    addToast("Ítem creado", "success");
    router.push("/dashboard/admin/library/items");
  }

  return (
    <div className="space-y-6">
      <BackLink href="/dashboard/admin/library/items" label="Volver al catálogo" />
      <h1 className="text-2xl font-bold text-text">Añadir ítem</h1>
      <Card className="max-w-2xl mx-auto p-6"><ItemForm onSubmit={handleSubmit} /></Card>
    </div>
  );
}
