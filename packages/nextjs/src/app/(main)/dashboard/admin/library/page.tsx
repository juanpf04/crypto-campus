...
interface LibraryStats {
  ...
  pendingRequests: number; activeLoans: number; overdueLoans: number;
}
    ...
    <div className="space-y-10">
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <StatCard
            ...
            value={val(ls?.pendingRequests)}
            subtitle={`${ls?.activeLoans ?? 0} activos, ${ls?.overdueLoans ?? 0} vencidos`}
            icon={icons.loans}
          />
          <StatCard
            title="Salas activas"
      ...
      <section className="space-y-4">
        ...
        <Card className="overflow-hidden p-0">
          <ActionRow
            href="/dashboard/admin/library/items"
            icon={icons.library}
            title="Catálogo"
          ...
          />
          <ActionRow
            href="/dashboard/admin/library/loans"
            icon={icons.loans}
            title="Préstamos"
            description="Gestionar solicitudes, aprobaciones y devoluciones"
            stat={`${ls?.pendingRequests ?? "—"} pendientes`}
          />
          <ActionRow
            href="/dashboard/admin/library/rooms"
            icon={icons.rooms}
          ...
          />
          <ActionRow
            href="/dashboard/admin/library/tokens"
            icon={icons.token}
            title="LibraryTokens"
