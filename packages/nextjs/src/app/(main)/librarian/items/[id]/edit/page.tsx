...
export default function LibrarianEditItemPage() {
    ...
    async function load() {
      try {
        ...
        setInitialValues({
          ...
          physicalCondition: item.physicalCondition || "Bueno",
          totalCopies: String(item.totalCopies),
          isbn: String(meta.isbn || ""),
          publisher: String(meta.publisher || ""),
          year: meta.year ? String(meta.year) : "",
          pages: meta.pages ? String(meta.pages) : "",
          players: String(meta.players || ""),
          duration: String(meta.duration || ""),
          ageRating: String(meta.ageRating || ""),
          platform: String(meta.platform || ""),
          genre: String(meta.genre || ""),
        });
      } catch {
        addToast("Error al cargar ítem", "danger");
        router.push("/librarian/items");
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="space-y-6">
      <BackLink href="/librarian/items" label="Volver al catálogo" />
      <h1 className="text-2xl font-bold text-text">Editar ítem</h1>
      <Card className="max-w-2xl mx-auto p-6">
        <ItemForm onSubmit={handleSubmit} initialValues={initialValues} isEdit />
      </Card>
    </div>
  );
}
