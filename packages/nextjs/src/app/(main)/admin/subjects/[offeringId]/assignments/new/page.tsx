 ...
 * Copia del formulario del profesor, con rutas base adaptadas.
 */
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card className="space-y-4">
          ...
          <Input label="Nombre" value={name} onChange={(e) => setName(e.target.value)} required placeholder="Práctica 1 - Página web" />
          <Textarea label="Descripción (opcional)" value={description} onChange={(e) => setDescription(e.target.value)} rows={3} placeholder="Detalles de la entrega, requisitos, etc." />
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Input label="Fecha límite (opcional)" type="datetime-local" value={deadline} onChange={(e) => setDeadline(e.target.value)} />
            <div className="flex flex-col justify-end gap-2">
              <Toggle label="Auto-cerrar al pasar la fecha" checked={autoClose} onChange={setAutoClose} disabled={!deadline} />
        ...
        <Card className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button type="button" variant="secondary" size="sm" onClick={addPrize}>+ Añadir premio</Button>
          </div>
            ...
            {prizes.map((prize, idx) => (
              <div key={idx} className="rounded-lg border border-border-default p-4 space-y-3 bg-bg">
                <div className="flex items-center justify-between">
                  ...
                  {prizes.length > 1 && (
                    <button type="button" onClick={() => removePrize(idx)} className="text-xs text-danger hover:underline cursor-pointer">
                      Eliminar
                    </button>
                  )}
                </div>
                <Input label="Nombre del premio" value={prize.name} onChange={(e) => updatePrize(idx, { name: e.target.value })} required placeholder="Mejor diseño, Mejor nota, Más útil..." />
                <Input label="Descripción (opcional)" value={prize.description} onChange={(e) => updatePrize(idx, { description: e.target.value })} placeholder="Criterio de evaluación de este premio" />
                <div className="grid grid-cols-2 gap-3">
                  <Input label="Insignias por ganador" type="number" min={1} value={prize.badgeReward} onChange={(e) => updatePrize(idx, { badgeReward: Number(e.target.value) })} />
                  <Input label="Nº máximo de ganadores" type="number" min={1} value={prize.maxWinners} onChange={(e) => updatePrize(idx, { maxWinners: Number(e.target.value) })} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button type="submit" loading={submitting}>Crear tarea</Button>
        </div>
      </form>
    </div>
  );
