...
/**
 * Solicitudes de uso de una asignatura (admin).
 */
...
export default function AdminOfferingUseRequestsPage() {
  ...
  function updateStatus(next: string | null) {
    ...
    router.replace(`/admin/subjects/${offeringId}/use-requests${qs ? `?${qs}` : ""}`);
  }
    ...
    <div className="space-y-6">
      ...
      {requests.length === 0 ? (
        <EmptyState
          ...
          description={statusParam ? "No hay solicitudes con este estado." : "Aún no hay solicitudes de uso en esta asignatura."}
        />
      ) : (
        <UseRequestsTable requests={requests} showSubjectColumn={false} onReload={load} />
      )}
    </div>
  );
}
