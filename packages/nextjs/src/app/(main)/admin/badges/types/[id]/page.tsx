...
export default function AdminBadgeTypeDetailPage() {
  ...
  if (!badgeType) {
    return (
      <div className="space-y-6">
        <BackLink href="/admin/badges/types" label="Volver a tipos de insignia" />
  ...
  const professorName =
    badgeType.creator?.name ??
    badgeType.subjectOffering?.professor?.name ??
    "—";
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-4 mt-3 text-sm text-text-muted">
          ...
          <span>Profesor: <strong className="text-text">{professorName}</strong></span>
        </div>
      </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Link href={`/admin/badges/types/${id}/tasks/new`}>
        ...
        {badgeType.tasks.length === 0 ? (
          <EmptyState
            title="Sin tareas"
            description="Aún no se ha creado ninguna tarea para este tipo de insignia."
          />
        ) : (
                        ...
                        {task.active && (
                          <>
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => router.push(`/admin/badges/types/${id}/tasks/${task.id}/award`)}
                            >
                              Otorgar
                            </Button>
                            <Button
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Link href="/admin/badges/rewards/new">
