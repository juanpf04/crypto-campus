...
/**
 * Crear tipo de insignia (admin).
 *
 * Carga TODAS las ofertas de asignatura (el admin ve todas)
 * y muestra el formulario de creación de badge type.
 */
...
export default function AdminNewBadgeTypePage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSubjectOfferings(
          data.map((so) => ({
            value: so.id,
            label: `${so.subject.name} (${so.academicYear}${so.group ? ` - ${so.group}` : ""})`,
          })),
        );
      }
    } catch {
  ...
  async function handleSubmit(data: BadgeTypeFormData) {
    ...
    router.push("/admin/badges/types");
  }
