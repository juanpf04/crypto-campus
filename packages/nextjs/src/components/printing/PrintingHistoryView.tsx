...
/**
 * PrintingHistoryView — Historial paginado de impresiones del usuario logueado.
 *
 * Reutilizado por estudiante y profesor. `basePath` controla el destino del
 * BackLink (volver a la home de impresión) y el href de cada fila (ir al detalle).
 */
...
interface PrintingHistoryViewProps {
  /** Prefijo de URL para las rutas internas de impresión (e.g. "/professor/printing"). */
  basePath: string;
}

const PAGE_SIZE = 10;

export function PrintingHistoryView({ basePath }: PrintingHistoryViewProps) {
  ...
  return (
    <div className="space-y-6">
      <BackLink href={basePath} label="Volver a impresión" />
              ...
              <TableBody>
                {list.items.map((log) => (
                  <TableRow
                    ...
                    onClick={() => router.push(`${basePath}/history/${log.id}`)}
                  >
