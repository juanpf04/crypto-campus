/**
 * seed-librarian.mjs — Crea el usuario bibliotecario por defecto.
 *
 * Es idempotente: si ya existe en la BD, no hace nada.
 *
 * Pasos:
 * 1. Comprueba si ya existe un usuario con email librarian@ucm.es
 ...
 * 6. Registra al bibliotecario en CampusRoles con LIBRARIAN_ROLE
 * 7. Guarda en PostgreSQL con rol LIBRARIAN
 *
 * Uso: node scripts/seed-librarian.mjs
 * Se ejecuta automáticamente desde scripts/seed.mjs.
 */
...
function log(msg) {
  console.log(`${cyan("[seed-librarian]")} ${msg}`);
}
...
const LIBRARIAN_ROLE_HASH = keccak256(toBytes("LIBRARIAN_ROLE"));
...
// ── Datos del librarian por defecto ──
const LIBRARIAN_EMAIL = "librarian@ucm.es";
const LIBRARIAN_PASSWORD = "Admin^12";
const LIBRARIAN_NAME = "Bibliotecario";
...
async function main() {
  ...
  try {
    ...
    const existing = await prisma.user.findUnique({ where: { email: LIBRARIAN_EMAIL } });
    if (existing) {
      log(green(`Librarian ya existe (${LIBRARIAN_EMAIL}). Saltando.`));
      return;
    }

    log("Creando usuario bibliotecario por defecto...");

    // 2. Hashear contraseña
    const hashedPassword = await bcrypt.hash(LIBRARIAN_PASSWORD, 10);
    ...
    await txWaitOrThrow(fundHash, "fund librarian wallet");

    // 6. Registrar en CampusRoles con LIBRARIAN_ROLE
    const regHash = await adminWalletClient.writeContract({
      address: ADDRESSES.campusRoles,
      abi: CAMPUS_ABI,
      functionName: "registerUser",
      args: [account.address, LIBRARIAN_NAME, LIBRARIAN_ROLE_HASH],
    });
    await txWaitOrThrow(regHash, "registerUser librarian");
    ...
    log(green(`Librarian creado: ${LIBRARIAN_EMAIL} / ${LIBRARIAN_PASSWORD}`));
  } finally {
